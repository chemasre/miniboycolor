	...
	var opCodes =
	{
		...
		screenSetClearColorR:	41,
		screenSetClearColorG:	42,
		screenSetClearColorB:	43,
		screenGetPixel:			44,
		screenGetPixelR:		45,
		screenGetPixelG:		46,
		screenGetPixelB:		47,
	...
	class Machine
	{
		...
		HelpStart()
		{
			...
			for(var i = 0; i < instructionInfo.length; i ++)
			{
				...
				text += MachineUtils.PadTextRight(instructionInfo[i].opCodeNames[0], 24);
		...
		ConfigurationShow()
		{
			...
			this.configurationVersionCell.value 			= "2.0";
		...
		// ProgramDoCompileInstruction(instructionIndex)
		// {
			// var line = this.programCells[instructionIndex].value;
			...
			// {
				...
				// {
					...
					// {
						// this.ProgramDoCompileArgument(instructionIndex, i, parts[i + 1]);
					// }
		...
		ProcessorTick()
		{
			...
			else // this.state == states.stateStopped
			{
				...
				if(this.applyConfigurationPressed || this.resetPressed || this.clearProgramPressed || this.clearMemoryLabelsPressed || this.newProjectPressed || this.loadProjectPressed || this.saveProjectPressed)
				{
					if(this.applyConfigurationPressed)
					{
						...
						if(result)
						{
							var result = confirm("Esta operación devolverá la máquina a su estado inicial, preservando sólo las etiquetas que hayas definido en la memoria estática y el programa ¿Estás seguro/a?");
							if(result)
							{
								// Preserve static memory labels and program
								...
								// Restore static memory labels and program
			...
			if(showStack)
			{
				...
				{				
					...
					var returnAddress = parseInt(currentBaseCell.value);
		...
		DoEnterStateRunning()
		{
			...
			// var programCompiled = true;

			// if(this.step)
			// {
				// // Compile only the required instruction
				
				// var pc = this.registers[registerId.pc];
				
				// programCompiled = this.ProgramDoCompileInstruction(pc);
			// }
			// else
			// {
				// // Compile the full program
				...
				// {
					...
					// lineCompiled = this.ProgramDoCompileInstruction(i);
					
					// programCompiled = programCompiled || lineCompiled;
				// }
			// }
